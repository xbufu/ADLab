function Invoke-ADLabDeploy {
    ...
    <#
        ...
        .EXAMPLE
            PS > Invoke-ADLabDeploy -Domain bufu-sec.local

            Installs a new forest with FQDN of "bufu-sec.local" with default DSRM password of "Password!".

        .EXAMPLE
            PS > Invoke-ADLabDeploy -Domain "bufu-sec.local" -DSRMPassword "P@ssword!" -Verbose
    ...
    Install-ADDSForest -DomainName "$Domain" -DomainNetbiosName "$NetbiosName" -SafeModeAdministratorPassword $Password -CreateDnsDelegation:$false -DatabasePath "C:\Windows\NTDS" -SysvolPath "C:\Windows\SYSVOL" -LogPath "C:\Windows\NTDS" -DomainMode "WinThreshold" -ForestMode "WinThreshold" -NoRebootOnCompletion:$false -InstallDNS:$true -Force:$true
}
